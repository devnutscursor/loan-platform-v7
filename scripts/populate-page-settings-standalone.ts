...
async function getLoanOfficers(): Promise<LoanOfficer[]> {
  console.log('🔍 Fetching loan officers...');
  ...
  console.log(`✅ Found ${result.length} loan officers`);
  return result;
}
...
async function getTemplates(): Promise<Template[]> {
  ...
  console.log(`✅ Found ${result.length} active templates`);
  return result;
}

async function getExistingPageSettings(): Promise<string[]> {
  console.log('🔍 Checking existing page settings...');
  ...
  console.log(`✅ Found ${officerIds.length} existing page settings`);
  return officerIds;
}
...
async function populatePageSettings() {
  try {
    console.log('🚀 Starting page settings population...');
    
    // Get all loan officers
    const loanOfficers = await getLoanOfficers();
    if (loanOfficers.length === 0) {
      console.log('⚠️ No loan officers found. Exiting.');
      return;
    }
    ...
    if (templates.length === 0) {
      console.log('⚠️ No templates found. Exiting.');
      return;
    }
    ...
    console.log(`📋 Using default template: ${defaultTemplate.name} (${defaultTemplate.slug})`);
    ...
    for (const officer of loanOfficers) {
      ...
      if (existingOfficerIds.includes(officer.id)) {
        console.log(`⏭️ Skipping officer ${officer.email} - already has page settings`);
        skippedCount++;
        continue;
      }

      try {
        console.log(`📝 Creating page settings for officer: ${officer.email}`);
        ...
        console.log(`✅ Created page settings for ${officer.email} (ID: ${newPageSettings.id})`);
        ...
        console.log(`✅ Created initial version for ${officer.email} (Version ID: ${newVersion.id})`);
        createdCount++;

      } catch (error) {
        console.error(`❌ Error creating page settings for ${officer.email}:`, error);
      }
    }

    console.log('\n📊 Summary:');
    console.log(`✅ Created: ${createdCount} page settings`);
    console.log(`⏭️ Skipped: ${skippedCount} officers (already have settings)`);
    console.log(`📋 Total officers processed: ${loanOfficers.length}`);

  } catch (error) {
    console.error('❌ Error in populatePageSettings:', error);
    throw error;
  }
}
...
async function main() {
  try {
    ...
    console.log('\n🎉 Page settings population completed successfully!');
  } catch (error) {
