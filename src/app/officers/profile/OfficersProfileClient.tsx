...
import type { OfficerPublicLink } from '@/lib/officer-public-link';
...
export type OfficersProfileClientProps = {
  initialPublicLink: OfficerPublicLink | null;
  initialPublicLinkFetched: boolean;
};

export default function OfficersProfileClient({
  initialPublicLink,
  initialPublicLinkFetched,
}: OfficersProfileClientProps) {
  ...
  // Track which user we've completed a public-link fetch for (avoids "No Public Link" flash before first fetch)
  const publicLinkFetchedForUserIdRef = React.useRef<string | null>(null);
  ...
  // Public link state: when server provided data, use it so no client fetch on load
  const [publicLink, setPublicLink] = useState<any>(initialPublicLinkFetched ? initialPublicLink : null);
  ...
  const fetchPublicLink = React.useCallback(async (userId: string, companyIdFromAuth?: string | null) => {
    try {
      ...
      if (companyIdFromAuth) {
        ...
        const companyResponse = await fetch(`/api/user-company?userId=${userId}`);
      ...
      const response = await fetch(`/api/public-links?userId=${userId}&companyId=${companyId}`);
      const result = await response.json();
      ...
      publicLinkFetchedForUserIdRef.current = userId;
    }
  }, []);
  ...
  // When server provided link data, mark as fetched so we don't show loading
  React.useEffect(() => {
    if (initialPublicLinkFetched && user?.id) {
      publicLinkFetchedForUserIdRef.current = user.id;
    }
  }, [initialPublicLinkFetched, user?.id]);

  // Fetch public link only when server did not provide it (client-side fallback or after create/deactivate)
  React.useEffect(() => {
    if (user?.id && !initialPublicLinkFetched) {
      fetchPublicLink(user.id, companyId ?? undefined);
    }
  }, [user?.id, initialPublicLinkFetched, fetchPublicLink]);
    ...
    try {
      ...
      if (!companyIdToUse) {
        ...
        if (!companyResult.success) {
          setPublicLinkError(companyResult.message);
          setPublicLinkLoading(false);
          setPublicLinkCreating(false);
          return;
        }
        companyIdToUse = companyResult.data.companyId;
      }
      ...
      >
        ...
        <div className="space-y-4 md:space-y-6 profile-page-container overflow-auto">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6 overflow-x-auto">
            ...
            {(publicLinkLoading || (user?.id && publicLink === null && !publicLinkError && publicLinkFetchedForUserIdRef.current !== user.id)) ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-500 mt-4">Loading your public link...</p>
              </div>
            ) : publicLink ? (
              ...
              <div className="text-center py-8">
                ...
                <Button
                  ...
                  variant="primary"
                >
                  {publicLinkCreating ? 'Creating...' : publicLinkLoading ? 'Loading...' : 'Create Public Link'}
                </Button>
              </div>
            )}
          </div>
